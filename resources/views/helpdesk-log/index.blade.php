<x-dashboard-shell title="Helpdesk Log" active="helpdesk-log">
                    ...
                    >
                        <x-slot:aside>
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <x-stat-card label="Baru" :value="$stats['baru']" hint="Belum diproses" />
                    ...
                    <x-filter-card :action="route('helpdesk-log.index')" class="grid gap-4 md:grid-cols-2 xl:grid-cols-[1fr_10rem_10rem_auto]">
                            ...
                            @forelse ($logs as $log)
                                <article class="grid grid-cols-12 gap-4 px-5 py-4 text-sm">
                                    <div class="col-span-12 md:col-span-3">
                                        ...
                                        <div class="mt-1 text-xs text-slate-500">{{ $log->pelapor_email }}</div>
                                        <div class="mt-1 text-xs text-slate-400">oleh {{ $log->user?->name ?? '-' }}</div>
                                    </div>
                                    ...
                                    <div class="col-span-6 md:col-span-2">
                                        @if ($log->status === 'baru')
                                            <span class="inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700">Baru</span>
